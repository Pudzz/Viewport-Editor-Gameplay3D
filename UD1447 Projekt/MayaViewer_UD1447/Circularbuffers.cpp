...
bool CircularBuffer::Recieve(char*& message, SectionHeader*& header)
{
    mutex->Lock();
    bool result = false;
    ...
    if (*freeMemory < sharedMemory->GetBufferSize())
    {
        if (*head != *tail)
        {
            ...
            header = ((SectionHeader*)&messageData[*tail]);
            msgLength = header->messageSize;

            std::cout << "MESSAGELENGTH IN SEND: " << msgLength << std::endl;
            if (header->ID == 0)
            {
